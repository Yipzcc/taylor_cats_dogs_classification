...
class ModifiedVGG16Model(torch.nn.Module):
    def __init__(self):
        ...
        model = models.vgg16(pretrained=False)#为加载的VGG16的模型
        self.features = model.features #提取VGG16的卷积层做特征提取层

        # for param in self.features.parameters():# 前面的参数做冻结，提取VGG16
    ...
    def forward(self, x): #VGG16的卷积层做特征提取层
        x = self.features(x) #展成一维向量作为fc层的输入
        x = x.view(x.size(0), -1)   #展成一维向量作为fc层的输入
        x = self.classifier(x) #输入到fc层中，得到最后的输出x
        return x

class FilterPrunner:#这个类主要实现：计算需要裁剪的filter
    ...
    def forward(self, x):#被PrunningFineTuner_VGG16类的train_batch调用
        self.activations = []
        self.gradients = []
        self.grad_index = 0
    ...
    def compute_rank(self, grad):#被forward调用，用于在grad更新时，计算值用于排序
    ...
    def lowest_ranking_filters(self, num):#被get_prunning_plan调用，计算最小的512个filter
        data = []
    ...
    def normalize_ranks_per_layer(self):#被PrunningFineTuner类调用，将每层的结果都归一化
...
class PrunningFineTuner_VGG16:#控制裁剪的整体过程：前向–》获取rank_filter–》计算prune_targets–》裁剪
    def __init__(self, train_path, test_path, model):
        self.train_data_loader = dataset.loader(train_path) #加载数据
        self.test_data_loader = dataset.test_loader(test_path) #加载数据
